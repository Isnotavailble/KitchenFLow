...
import { Store, ChefHat, LogOut, User } from 'lucide-react'
...
export default function MenuHeader() {
  ...
  return (
    <header className="sticky top-0 bg-white border-b border-zinc-200/80 shadow-[0_2px_8px_rgba(0,0,0,0.04)] px-6 py-3 flex items-center justify-between shrink-0 z-30 select-none">
      {/* Brand Logo & Title (Clean, no subtitle) */}
      <div className="flex items-center space-x-3">
        <div className="w-10 h-10 rounded-xl overflow-hidden shadow-2xs border border-orange-200/80 shrink-0 flex items-center justify-center bg-[#FF5C39]">
          <img src={pageLogo} alt="KitchenFlow" className="w-full h-full object-cover" />
        </div>
        <h1 className="text-xl font-bold tracking-tight text-zinc-900 leading-none font-sans">
      ...
      <div className="flex items-center space-x-2.5">
        <Link
          to="/pos"
          className="h-8 flex items-center space-x-1.5 px-3.5 bg-white hover:bg-zinc-50 border border-zinc-200/90 rounded-xl text-xs font-semibold text-zinc-700 shadow-xs transition active:scale-[0.96] shrink-0"
        >
