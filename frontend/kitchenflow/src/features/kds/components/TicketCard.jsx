...
export default function TicketCard({ ticket }) {
  ...
  return (
    <>
      <div className={`bg-white rounded-2xl border ${isCancelled ? 'border-rose-200 bg-rose-50/10' : isCompleted ? 'border-zinc-200/60' : 'border-zinc-200/80'} shadow-xs p-4 sm:p-5 flex flex-col h-[440px] select-none hover:shadow-sm transition-shadow min-w-[280px]`}>
        ...
        <div className="pt-2 shrink-0">
          {isCancelled ? (
            ...
            /* Owner: Both Cancel and Mark Complete Actions with micro-animations */
            <div className="flex items-center space-x-2">
              <button
                type="button"
                disabled={isProcessing}
                onClick={() => setIsCancelModalOpen(true)}
                className="group flex-1 py-2.5 bg-white hover:bg-rose-50 border border-rose-200 text-rose-600 font-bold text-xs rounded-xl shadow-xs transition-all duration-200 ease-out hover:-translate-y-0.5 hover:shadow-sm hover:shadow-rose-500/15 active:translate-y-0 active:scale-[0.95] flex items-center justify-center space-x-1.5 cursor-pointer disabled:opacity-50"
                title="Cancel this order (Owner authority)"
              >
                {isProcessing ? (
                  <Loader2 className="w-3.5 h-3.5 animate-spin shrink-0" />
                ) : (
                  <X className="w-3.5 h-3.5 shrink-0 transition-transform duration-200 ease-out group-hover:rotate-90 group-hover:scale-110" />
                )}
                <span>Cancel</span>
              </button>
              ...
              <button
                ...
                className="group flex-[1.4] py-2.5 bg-[#FF5C39] hover:bg-[#F04D28] text-white font-bold text-xs rounded-xl shadow-xs transition-all duration-200 ease-out hover:-translate-y-0.5 hover:shadow-md hover:shadow-orange-500/25 active:translate-y-0 active:scale-[0.95] flex items-center justify-center space-x-1.5 cursor-pointer disabled:opacity-50"
              >
                <span>Complete</span>
                <Plus className="w-3.5 h-3.5 shrink-0 transition-transform duration-200 ease-out group-hover:rotate-90 group-hover:scale-110" />
              </button>
            </div>
      ...
      {/* Owner Cancel Confirmation Modal */}
      <CancelOrderModal
