...
import { Store, LogOut, User, UtensilsCrossed } from 'lucide-react'
...
export default function KdsHeader() {
  ...
  return (
    <header className="bg-white border-b border-zinc-200/80 shadow-[0_2px_8px_rgba(0,0,0,0.04)] px-6 py-3.5 flex items-center justify-between select-none shrink-0 z-10">
      {/* Left: Clean Main Brand with Logo */}
      ...
      {/* Right: Switcher Links, User & Logout */}
      <div className="flex items-center space-x-2.5">
        {/* Owner Menu Management Link */}
        {isAdmin && (
          <Link
            to="/menu"
            ...
            title="Manage Menu Catalog and Dish Photos"
          >
        ...
        {(isAdmin || user?.role === 'ROLE_CASHIER') && (
          <Link
            to="/pos"
            className="h-8 flex items-center space-x-1.5 px-3.5 bg-orange-50 hover:bg-orange-100/80 text-[#C2410C] border border-orange-200/70 rounded-xl text-xs font-semibold transition active:scale-[0.96] shrink-0"
